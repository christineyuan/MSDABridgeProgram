...
namedlist <- list(rank = 1:4, family = c("Christopher", "Christine","Lily","Charles"))
...
services <- as.character(c("Benefit Verification","Prior Authorization", "PAP",
             "PA Appeal", "PAP Appeal",
             "Predetermination", "Formulary Exception", "Copay",
             "Copay Appeal", "Orders"))
serviceline <- c("Access", "Access", "Affordability","Affordability", "Engagement", 
                 "Affordability", "Affordability", "Affordability", "Affordability", "Engagement")
reports <- as.numeric(c(4, 8, 5, 7, 7, 5, 6, 3, 3, 6))
reportdate <- as.Date(c("1992-1-1", "1978-1-1", "1934-1-1", "1957-1-1", "1962-1-1",
          "1991-1-1", "1974-1-1", "1943-1-1", "1972-1-1", "1950-1-1"))

df <- data.frame(services, serviceline, reports, reportdate)
df$serviceline <- as.character(df$serviceline)
str(df)
...
dfnew <- data.frame(services="Predetermination Appeal", serviceline="Access", 
                  reports=10, reportdate="1961-1-1")
str(dfnew)
dfnew$reportdate <- as.Date(dfnew$reportdate)
dfnew$services <- as.character(dfnew$services)
...
# alternative:
df <- read.csv(file = "C:/Users/christine.yuan/Documents/2016_MSDA/Flights/airlines.csv")
df
...
years <- 6
compounding_periods <- compounding_periods_per_year * years
